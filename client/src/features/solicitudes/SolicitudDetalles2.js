...
const SolicitudDetalles2 = () => {
  const { id_solicitud } = useParams();
  const { data: solicitud, isFetching, isSuccess } = useGetSolicitudesAceptadasQuery(id_solicitud);
  ...
  if (isFetching) {
    ...
    return <div>No se encontraron detalles para esta solicitud.</div>;
  }
}

export default SolicitudDetalles2;
