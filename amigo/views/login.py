    ...
    def incrementoFallo(self, request):
        if 'login_failed_attempts' not in request.session:
            ...
            if errores == 3:
                ...
                return Response({
                    "error": f"{request.session.get('block_time')} y {int(request.session.get('login_failed_attempts'))+1} {type(request.session.get('login_failed_attempts'))}",
                    "tiempo": 60}, 
                    status=status.HTTP_400_BAD_REQUEST
                )
            elif errores > 3:
                ...
                if block_time_str:
                    ...
                    if timezone.now() > block_time + timedelta(seconds=60):
                        request.session['login_failed_attempts'] = 1
                        return Response({"error": f"{request.session.get('block_time')} y {int(request.session.get('login_failed_attempts'))+1} {type(request.session.get('login_failed_attempts'))}"}, status=status.HTTP_400_BAD_REQUEST)
                    else:
